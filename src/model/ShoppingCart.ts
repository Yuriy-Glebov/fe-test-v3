    ...
    handleOffers(receipt: Receipt,  offers: OffersByProduct, catalog: SupermarketCatalog ):void {
        for (const productName in this.productQuantities()) {
            ...
            if (offers[productName]) {
                ...
                let discountGroupSize;
                let numberOfDiscountGroups;
                ...
                if (offer.offerType == SpecialOfferType.TwoForAmount && quantityAsInt >= 2) {
                    discountGroupSize = 2;
                    const total = offer.argument * Math.floor(quantityAsInt / discountGroupSize) + quantityAsInt % 2 * unitPrice;
                ...
                if (offer.offerType == SpecialOfferType.ThreeForTwo && quantityAsInt >= 3) {
                    discountGroupSize = 3;
                    numberOfDiscountGroups = Math.floor(quantityAsInt / discountGroupSize);
                    const discountAmount = quantity * unitPrice - ((numberOfDiscountGroups * 2 * unitPrice) + quantityAsInt % 3 * unitPrice);
                    discount = new Discount(product, "3 for 2", discountAmount);
                }
                if (offer.offerType == SpecialOfferType.FiveForAmount && quantityAsInt >= 5) {
                    discountGroupSize = 5;
                    numberOfDiscountGroups = Math.floor(quantityAsInt / discountGroupSize);
                    const discountTotal = unitPrice * quantity - (offer.argument * numberOfDiscountGroups + quantityAsInt % 5 * unitPrice);
                    discount = new Discount(product, discountGroupSize + " for " + offer.argument, discountTotal);
                }
                if (discount != null)
